...
### Decision 6：焚香中途操作——熄香與跳過
**選擇**：提供「熄香」與「跳過，繼續儀式」兩個操作

**理由**：
- 現實中使用者可能提前熄香或不等香燒完就進行下一步驟
- 「熄香」停止計時並保留當前香體長度，進入完成狀態
- 「跳過」直接進入下一步，焚香在背景繼續計時，計時結束仍會推播通知
- 兩者語意不同：熄香是主動結束儀式環節，跳過是不想等但香仍在燒
